...
interface DataTableProps<T> {
    ...
    isRowDownloadable?: (item: T) => boolean;
    onClickDownload: (selectedItems: T[]) => void;
}

const DataTable = <T extends { id: number | string }>({
    data,
    columns,
    isRowDownloadable = () => true,
    onClickDownload,
}: DataTableProps<T>) => {
    const [selectedRows, setSelectedRows] = useState<T["id"][]>([]);

    const isAllSelected = selectedRows.length === data.length;
    const isIntermediateCheckState =
        selectedRows.length > 0 && selectedRows.length < data.length;

    const canDownload = selectedRows.every((id) =>
        isRowDownloadable(data.find((item) => item.id === id)!)
    );
            ...
            <div className={Styles.header}>
                <div className={Styles.checkboxContainer}>
                    <input
                        ...
                        ref={(el) => {
                            if (el) {
                                el.indeterminate = isIntermediateCheckState;
                            }
                        }}
                ...
                <button
                    ...
                    disabled={!canDownload || selectedRows.length === 0}
